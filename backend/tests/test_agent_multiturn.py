...
async def test_broad_question_starts_with_monthly_trend_and_reuses_it():
    trend_result = {
        "status": "success",
        "grain": "month",
        "split_by": None,
    ...
    assert [name for name, _ in mcp.calls] == ["sales_trend"]
    ...
    assert first.visualizations[0].type == "line_chart"
    assert response.visualizations[0].type == "line_chart"
    assert response.visualizations[0].y_keys == ["net_sales"]
    assert response.message == "Här är resultatet som diagram."
