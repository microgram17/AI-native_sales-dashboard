...
def create_sales_tools(ctx: SalesToolContext) -> list[Callable]:
    ...
    analytics_port, collected, tools_used). Cross-turn follow-up filter state is
    written to ADK session.state via tool_context.state. supplier_id is never
    ...
    async def get_sales_summary(
        ...
        tool_context.state["last_filters"] = {"date_from": date_from, "date_to": date_to}
        tool_context.state["last_tool_used"] = "get_sales_summary"
    ...
    async def get_product_timeseries(
        ...
        date_from, date_to, was_clamped = clamp_dates(date_from, date_to, ctx.runtime_context)
        tool_context.state["last_filters"] = {"date_from": date_from, "date_to": date_to, "metric": metric, "grain": grain, "limit": limit}
        tool_context.state["last_tool_used"] = "get_product_timeseries"
    ...
    async def get_top_products(
        ...
        date_from, date_to, was_clamped = clamp_dates(date_from, date_to, ctx.runtime_context)
        tool_context.state["last_filters"] = {"date_from": date_from, "date_to": date_to, "sort_by": sort_by, "limit": limit}
        tool_context.state["last_tool_used"] = "get_top_products"
    ...
    async def get_store_breakdown(
        ...
        if group_by not in {"store", "city", "channel"}:
            group_by = "store"
        date_from, date_to, was_clamped = clamp_dates(date_from, date_to, ctx.runtime_context)
        tool_context.state["last_filters"] = {"date_from": date_from, "date_to": date_to, "metric": metric, "group_by": group_by}
        tool_context.state["last_tool_used"] = "get_store_breakdown"
    ...
    async def get_ranked_products(
        ...
        date_from, date_to, was_clamped = clamp_dates(date_from, date_to, ctx.runtime_context)
        tool_context.state["last_filters"] = {
            "date_from": date_from, "date_to": date_to,
            "metric": metric, "limit": limit,
            "city": city, "store_id": store_id,
            "channel": channel, "category": category,
        }
        tool_context.state["last_tool_used"] = "get_ranked_products"
    ...
    async def get_ranked_locations(
        ...
        tool_context.state["last_tool_used"] = "get_ranked_locations"
